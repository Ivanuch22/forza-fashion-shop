...
const PromoTag = ({ code, checkoutId }: { checkoutId: string, code: string }) => {
  ...
  const onSubmit = async () => {
    ...
    if (checkoutRemovePromoCode?.errors?.length > 0) {
      setErrors(checkoutRemovePromoCode?.errors)
    } else {
      setErrors([])
    }
    ...
    <>
      <div className="inline-flex items-center border border-gray-300 rounded-md px-2 py-1 bg-gray-100 text-gray-700 text-xs ">
        <span className="flex items-center gap-2">
          <span className="text-sm w-[18px] h-[18px] ">
            <svg className="w-full h-full stroke-current " xmlns="http://www.w3.org/2000/svg" viewBox="0 0 14 14" focusable="false" aria-hidden="true" >
              <path
                strokeLinecap="round"
      ...
      {errors?.length > 0 &&
        (
          <div>
            {errors.map((error) => (
              <p key={`${error?.message}+${error?.code}`} className="text-[14px] text-red-600 py-1">{error?.message}</p>
            ))}
          </div>
        )}
    </>
...
export const CheckoutSummary = ({ cart }: { cart: CheckoutFindQuery["checkout"] }) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setPromocode("")
    setLoading(false)
    if (checkoutAddPromoCode?.errors?.length > 0) {
      setErrors(checkoutAddPromoCode?.errors)
    }
  ...
  return (
    <div className="p-1">
      ...
      {cart?.voucherCode &&
        <PromoTag checkoutId={cart.id} code={cart?.voucherCode || ""} />
      }
      {errors?.length > 0 &&
        (
          <div>
      ...
      <div className="mt-6 flex justify-between items-center text-sm text-black mb-2">
        <span>Subtotal • {cart?.lines.reduce((acc, line) => acc + line?.quantity, 0) ?? 0} items</span>
        <span>{formatMoney(cart?.totalPrice?.gross?.amount || 0, cart?.totalPrice?.gross?.currency || "")}</span>
      </div>
      {/* Shipping Section */}
      ...
      <div className="flex justify-between items-end font-semibold text-lg text-gray-900 mt-4">
        ...
        <div className="text-right flex items-end font-semibold gap-2 ">
          <span className="pb-[4px] block font-semibold text-sm text-gray-500">
            {cart?.totalPrice?.gross?.currency}
          </span>
          <span>
      ...
      {(cart?.discount?.amount != 0 && cart?.discount?.amount) &&
        <div className="text-sm font-semibold mt-2 flex items-center justify-start gap-1">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18" focusable="false" aria-hidden="true" className="w-[18px] h-[18px] block a8x1wu10 a8x1wuz _1fragem1y _1fragemod _1fragemkk _1fragemka _1fragemnm"><path strokeLinecap="round" strokeLinejoin="round" d="m10.802 15.686 5.367-5.368a.15.15 0 0 0 .046-.109V4.144m-2.998-.95h-5.67a.16.16 0 0 0-.11.046L1.779 8.897a.154.154 0 0 0 0 .219l5.594 5.593c.06.06.158.06.218 0l5.658-5.657a.15.15 0 0 0 .045-.11v-5.67a.077.077 0 0 0-.077-.077Zm-3.06 3.749a.643.643 0 1 1-1.286 0 .643.643 0 0 1 1.286 0m-.648-.005h.01v.01h-.01z"></path>
          </svg>
          TOTAL SAVINGS {formatMoney(cart?.discount?.amount || 0, cart?.discount?.currency || "")}
        </div>
      }
    </div>
  );
